...
def enrichment_node(state: IntelligenceState) -> dict:
    bills = state["bills"]
    subscriptions = state["subscriptions"]

    if not bills and not subscriptions:
        return {"enrichment_result": {"bills": [], "subscriptions": []}}
    ...
    enrichment_by_bill_id = {
        e["id"]: e["enrichment"]
        for e in enrichment.get("bills", [])
        if "id" in e and "enrichment" in e
    }
    enrichment_by_sub_id = {
        e["id"]: e["enrichment"]
        for e in enrichment.get("subscriptions", [])
        if "id" in e and "enrichment" in e
    }
