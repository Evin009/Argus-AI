...
def test_bills_in_window_filters_correctly():
    today = date.today()
    bills = [
        {"next_due_date": (today + timedelta(days=1)).isoformat(), "avg_amount": 50.0, "merchant": "A"},
        {"next_due_date": (today + timedelta(days=10)).isoformat(), "avg_amount": 200.0, "merchant": "B"},
        {"next_due_date": (today + timedelta(days=40)).isoformat(), "avg_amount": 100.0, "merchant": "C"},
    ]
...
def test_compute_pay_timing_card_recommendation():
    accounts = [
        {"id": "card-1", "account_type": "credit", "balance": 800.0, "credit_limit": 1000.0, "closing_date": None}
    ]
...
def test_compute_pay_timing_skips_non_credit():
    accounts = [
        {"id": "chk-1", "account_type": "checking", "balance": 2000.0, "credit_limit": None, "closing_date": None}
    ]
    result = compute_pay_timing(accounts=accounts, bills=[], balance=2000.0)
...
def test_compute_pay_timing_stacked_window_detection():
    today = date.today()
    bills = [
        {"next_due_date": (today + timedelta(days=1)).isoformat(), "avg_amount": 300.0, "merchant": "Rent"},
        {"next_due_date": (today + timedelta(days=2)).isoformat(), "avg_amount": 200.0, "merchant": "Car"},
        {"next_due_date": (today + timedelta(days=3)).isoformat(), "avg_amount": 150.0, "merchant": "Electric"},
    ]
...
def test_no_stack_when_balance_covers_bills():
    today = date.today()
    bills = [
        {"next_due_date": (today + timedelta(days=1)).isoformat(), "avg_amount": 100.0, "merchant": "Netflix"},
    ]
