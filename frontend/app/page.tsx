  ...
  return (
    <div className="relative h-32 overflow-hidden rounded-md border border-border bg-surface-lowest">
      ...
      <div className="absolute inset-0 flex items-end px-2 pb-1">
        <svg viewBox="0 0 300 100" className="h-20 w-full opacity-70" preserveAspectRatio="none">
          <defs>
          ...
          <path d="M0,58 L25,50 L50,54 L75,38 L100,43 L125,28 L150,34 L175,20 L200,27 L225,14 L250,20 L275,10 L300,6 L300,100 L0,100 Z" fill="url(#chart-fill)" />
          <polyline points="0,58 25,50 50,54 75,38 100,43 125,28 150,34 175,20 200,27 225,14 250,20 275,10 300,6" fill="none" stroke="#ecbca7" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </div>
    </div>
  );
  ...
  return (
    <div className="h-32 overflow-hidden rounded-md border border-border bg-surface-lowest p-3 font-display">
      <div className="text-[11px] leading-relaxed">
        ...
        <div className="text-on-surface/40">› Overdraft risk: <span className="text-positive/60">LOW (4%)</span></div>
...
function HeatmapPreview() {
  const opacities = [0.8,0.2,0.9,0.1,0.5,0.7,0.3,0.6,0.4,0.8,0.1,0.7,0.9,0.2,0.5,0.3,0.6,0.8,0.4,0.9,0.2,0.7,0.5,0.1,0.8,0.3,0.6,0.9,0.4,0.7,0.2,0.5];
  return (
    <div className="flex h-32 items-center justify-center overflow-hidden rounded-md border border-border bg-surface-lowest">
      <div className="grid grid-cols-8 gap-1">
        {opacities.map((op, i) => (
          <div key={i} className="h-4 w-4 rounded-sm bg-accent" style={{ opacity: op }} />
        ))}
      </div>
    </div>
  );
...
function DashboardMockup() {
  ...
  const navItems = ["Dashboard","Accounts","Transactions","Cashflow","Risk Radar","Copilot","Goals"];
    ...
    <div className="overflow-hidden rounded-t-xl border-x border-t border-border shadow-2xl">
      ...
      <div className="flex h-[360px] bg-surface-lowest">
        <div className="flex w-44 flex-col gap-0.5 border-r border-border bg-surface px-2 py-4">
          <span className="mb-3 px-2 font-display text-xs font-bold uppercase tracking-widest text-accent-light">ArgusAI</span>
          {navItems.map((item, i) => (
            <div key={item} className={`rounded px-2 py-1.5 font-display text-xs ${i === 0 ? "border-l-2 border-accent bg-accent-container/30 text-accent-light" : "text-on-muted"}`}>
              {item}
            </div>
          ))}
        </div>
          ...
          <div className="mb-4 grid grid-cols-4 gap-3">
            {stats.map(({ label, value, delta, pos }) => (
              <div key={label} className="rounded-lg border border-border bg-surface p-2.5">
                <div className="mb-1 font-display text-[9px] uppercase tracking-widest text-on-muted">{label}</div>
                <div className="font-display text-sm font-bold text-on-surface">{value}</div>
                <div className={`mt-0.5 text-[9px] ${pos === true ? "text-positive" : pos === false ? "text-danger" : "text-on-muted"}`}>{delta}</div>
              </div>
            ))}
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div className="col-span-2 rounded-lg border border-border bg-surface p-3">
              <div className="mb-2 font-display text-[9px] uppercase tracking-widest text-on-muted">30-Day Cashflow Forecast</div>
              <svg viewBox="0 0 200 60" className="h-14 w-full" preserveAspectRatio="none">
                <defs>
                  <linearGradient id="mockup-grad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#ecbca7" stopOpacity="0.3" />
                    <stop offset="100%" stopColor="#ecbca7" stopOpacity="0" />
                  </linearGradient>
                </defs>
                <path d="M0,42 L20,36 L40,40 L60,26 L80,30 L100,20 L120,23 L140,15 L160,18 L180,9 L200,7 L200,60 L0,60 Z" fill="url(#mockup-grad)" />
                <polyline points="0,42 20,36 40,40 60,26 80,30 100,20 120,23 140,15 160,18 180,9 200,7" fill="none" stroke="#ecbca7" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
            <div className="rounded-lg border border-border bg-surface p-3">
              <div className="mb-2 font-display text-[9px] uppercase tracking-widest text-on-muted">Risk Radar</div>
              <div className="space-y-1.5">
                <div className="rounded border-l-2 border-warning bg-warning/10 px-2 py-1.5">
                  <div className="font-display text-[9px] font-semibold text-warning">Medium Risk</div>
                  <div className="text-[8px] text-on-muted">Credit util at 28%</div>
                </div>
                <div className="rounded border-l-2 border-positive bg-positive/10 px-2 py-1.5">
                  <div className="font-display text-[9px] font-semibold text-positive">Low Risk</div>
                  <div className="text-[8px] text-on-muted">Overdraft unlikely</div>
                </div>
              </div>
            </div>
          </div>
    ...
    <div className="min-h-screen bg-background text-on-background">
        ...
        <div className="flex h-14 items-center justify-between px-6">
          ...
          <div className="hidden items-center gap-8 font-display font-medium tracking-tight md:flex">
            {["Platform", "Features", "Security", "Docs"].map((link) => (
              <a key={link} href="#" className="text-on-surface/60 transition-colors hover:text-accent-light">
                {link}
              </a>
            ))}
          </div>
      ...
      <AuroraBackground className="relative flex h-screen w-full flex-col items-center justify-center px-6 text-center">
        <div className="absolute inset-0 z-0">
          <Image src="/images/hero.jpg" alt="" fill priority className="object-cover object-center" sizes="100vw" />
        ...
        <motion.div
          className="relative z-10 mb-[18vh] mt-16 max-w-4xl space-y-8"
        ...
        >
          ...
          <motion.p
            variants={heroItem}
            className="text-shadow-hero mx-auto max-w-2xl text-lg font-medium leading-relaxed text-on-surface/85"
          >
          ...
          <motion.div variants={heroItem} className="flex flex-col items-center justify-center gap-4 pt-4 sm:flex-row">
            <Button asChild size="lg">
              <Link href="/signup">Get Early Access</Link>
            </Button>
            <Button asChild variant="outline" size="lg" className="border-on-surface/20 bg-surface-lowest/60 backdrop-blur-md hover:bg-surface/60">
        ...
        >
          <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 2, ease: "easeInOut" }}>
            <svg className="h-6 w-6 text-on-surface/30" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <section className="relative z-10 -mt-[20vh] bg-transparent px-4 pb-24">
        <FadeIn className="mx-auto max-w-5xl drop-shadow-[0_-8px_32px_rgba(0,0,0,0.6)]">
          <DashboardMockup />
      ...
      {/* ── Feature Grid ──────────────────────────────────────────── */}
      <section id="features" className="border-y border-border/30 bg-surface-low px-6 py-24">
        <div className="mx-auto max-w-7xl">
          <FadeIn className="mb-16 text-center">
            <h2 className="font-display text-4xl font-bold text-on-surface">Tactical Intelligence</h2>
            <p className="mt-4 text-on-surface/55">Precision-engineered modules for total financial oversight.</p>
          </FadeIn>
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
            {features.map(({ title, description, previewType }, i) => (
              <FadeIn key={title} delay={i * 0.08}>
                <motion.div
                  whileHover={{ y: -6, transition: { duration: 0.22, ease: "easeOut" } }}
                  className="rounded-lg border border-border bg-surface p-8 transition-colors hover:border-accent/50 cursor-default"
                >
                  <h3 className="mb-4 font-display text-2xl font-bold text-on-surface">{title}</h3>
                  <p className="mb-6 text-sm leading-relaxed text-on-surface/55">{description}</p>
      ...
      <section className="mx-auto max-w-7xl px-6 py-24">
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {teasers.map(({ title, description }, i) => (
            <FadeIn key={title} delay={i * 0.07}>
              <motion.div
                whileHover={{ y: -4, transition: { duration: 0.2, ease: "easeOut" } }}
                className="rounded-lg border border-border bg-surface-low p-6 cursor-default"
              >
                <h4 className="mb-2 font-display font-bold text-on-surface">{title}</h4>
          ...
          <div className="relative mx-auto max-w-5xl overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-accent-container/30 to-surface p-12 text-center">
            ...
            <div className="mt-8 flex flex-col items-center justify-center gap-4 sm:flex-row">
              <Input
                ...
                className="w-full sm:w-80 bg-surface-low border-border text-on-surface placeholder:text-on-muted focus-visible:ring-accent"
              />
        ...
        <div className="mx-auto flex max-w-[1440px] flex-col items-center justify-between gap-8 px-6 py-12 md:flex-row md:px-12">
          ...
          <div className="flex flex-wrap justify-center gap-8 font-display text-[11px] uppercase tracking-widest">
            {["Features", "Pricing", "Privacy", "Terms", "Docs"].map((link) => (
              <a key={link} href="#" className="text-on-muted transition-colors hover:text-on-surface">
                {link}
              </a>
            ))}
          </div>
